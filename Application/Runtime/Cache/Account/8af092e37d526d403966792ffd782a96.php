...
var admission={
	...
	<li>
		<a href="<?php echo U('YaoHresult/index');?>"><?php echo ((isset($classify_name) && ($classify_name !== ""))?($classify_name):''); ?></a>
	</li>
	...
	<div class="col-xs-12">
		...
		<div class="table-responsive dataTables_wrapper">
			<div class="row">
				<div class="col-sm-12">
					<div id="sample-table-2_length" class="dataTables_length">
						<form action="<?php echo U('/Account/YaoHresult/index');?>" method="get" id="form-p">
						<label>
							项目 
						...
						<form action="<?php echo U('/Account/YaoHresult/index');?>" method="get" id="form-b">
						<label>							
							批次 
						...
						<label>
							入场状态
							<select name="status" class="yaohresult-status " style="width: auto;" id="jl_status">
							...
							<div class="nav-search" id="sample-table-2_filter"  style="top:0;">
								<label class="input-icon">
									<input type="text" name="word" value="<?php echo ((isset($search_word) && ($search_word !== ""))?($search_word):''); ?>" id="jl_ss"  class="nav-search-input" placeholder="客户名称、VIP编号、手机、证件号码">
			...
			<table id="sample-table-choose" class="table table-striped table-bordered table-hover dataTable">
				...
				<tbody>
					<?php if(!empty($list)): if(is_array($list)): $list_k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list_vo): $mod = ($list_k % 2 );++$list_k;?><tr class="yaohresult-item yaohresult-item-<?php echo ((isset($list_vo["id"]) && ($list_vo["id"] !== ""))?($list_vo["id"]):'0'); ?>">
								<td class="center hidden-480">
									<label>
										<input type="checkbox" class="ace yaohresult-item-id" data-id="<?php echo ((isset($list_vo["id"]) && ($list_vo["id"] !== ""))?($list_vo["id"]):'0'); ?>">
										<span class="lbl"></span>
									</label>
								</td>

								<td class="center hidden-480"><?php echo ($list_k); ?></td>
                                                                <td class="hidden-480">
                                                                    <span class="label-sm">
								...
								<td class="hidden-480">
                                                                    <?php echo ($list_vo["customer_name"]); ?>
								</td>
                                                                <td class="hidden-480">
                                                                    <?php echo ($list_vo["cyjno"]); ?>
								</td>
								<td class="hidden-480">
                                                                    <?php echo rsa_decode($list_vo['customer_phone'],getChoosekey());?>
								</td>
								<td>
                                                                     <?php echo rsa_decode($list_vo['cardno'],getChoosekey());?>
								</td>
                                                                <td>
                                                                    第<span style="font-weight: 700;color:red;"><?php echo ($list_vo["group"]); ?></span>组
								</td>
                                                                <td>
                                                                    <?php echo ($list_vo["no"]); ?>
								</td>
								<td class="hidden-480" style="text-align: center;">
									<?php if(!empty($list_vo['createdtime'])): echo (date("Y-m-d H:i:s",$list_vo["createdtime"])); endif; ?>
								</td>
								<td class="hidden-480 center">
									<?php if(($list_vo['status']) == "1"): ?><span class="label label-sm label-success">已入场</span>
									<?php else: ?>
										<span class="label label-sm label-warning">未入场</span><?php endif; ?>
								</td>
                                                                <td class="hidden-480 center">
									<?php if(($list_vo['is_yx']) == "1"): ?><span class="label label-sm label-success">正常</span>
									<?php else: ?>
										<span class="label label-sm label-grey">已作废</span><?php endif; ?>
								</td>
								<td style="text-align: center;">
                                                                    <!--<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
                                                                        <?php if($list_vo['is_yx'] == 1): ?><button data-id="<?php echo ($list_vo["id"]); ?>" class="js-yaohresult-circle-btn" title="作废" style="border-style: none;background-color: #ffb752;padding: 3px 7px">
