...
class YaoHresultController extends BaseController {
    ...
    public function check_jl(){
        ...
        $list = $yaohresult->getList(
            $where, "*,case when status=0 then '未入场' else '已入场' end as status1,case when is_yx=1 then '正常' else '已作废' end as is_yx1,FROM_UNIXTIME( createdtime,'%Y-%m-%d  %H:%i') as createdtime1", 'id desc'
        );
