...
class OrderHouseController extends BaseController
{
	...
	public function _initialize()
	{
	    ...
	    if (empty($privilege) && !in_array(ACTION_NAME,array('privilege','send_sms_code','check'))){
                session('order_house_jump',$_SERVER['REQUEST_URI']);
                $this->redirect("privilege",['eventId'=>I('id', 0, 'trim')]);
            }

	    if ($privilege==1 && in_array(ACTION_NAME,array('privilege','send_sms_code','check')))
	        $this->redirect('index',[id=>I('eventId', 0, 'trim')]);
            
            if(session('phone'))
            {
                if (cookie('eventId'))
                {
                    $eventInfo=D("EventOrderHouse")->getOneById(cookie('eventId'));
                    $where = [
                        'fields' => '*'
                        , 'where' => [
                            'customer_phone' => session('phone')
                            ,'project_id' => $eventInfo['project_id']
                            ,'batch_id' => $eventInfo['batch_id']
                            , '2=2'
                        ]
                    ];
                    $oneuser=M("choose")->find($where);
                    if(!$oneuser || $oneuser['status']==0)
                    {
                        $id = I('id',0);
                        session_unset();
                        session_destroy();
            ...
            if($eid && $cphone){
                ...
                if($event)
                {
                    ...
                    if(!$redisDriver->get("dlsx_order_house_{$eid}_{$cphone}")){//登录已失效
                        $this->logout();
                    }
                    else {
                        if (session_id()<> $redisDriver->get("dlsx_order_house_{$eid}_{$cphone}"))//同一手机号只能一台手机登录
                        {
                            $this->logout();
                        }
                    }
    ...
    public function index()
    {
        ...
        $orderedRoom = $orderHouseOrderModel->find(array(
            'where'=>array('event_id'=>$id,'belong_phone'=>session('phone'))
        ));
        ...
        //默认为 budings[0] unit[0] 下属单元 默认家在
    ...
    public function collectedroom()
    {
        ...
        foreach ($roomCollected as $key=>$item) {
            ...
            else
            {
                ...
                if (session('phone')==$room['schedule_phone'] && !$orderedRoom)
                {
    ...
    */
    public function room()
    {
        ...
        //获取该单元所有房间
        $eventOrderHouseModel = D('Common/EventOrderHouse');
        $unitKey = "event_order_house_{$condition['event_id']}_build_{$condition['build_id']}_unit_{$condition['unit_id']}";
    ...
    public function order()
    {
        ...
        //用户预购房间
        $userygroom;
        $evrnt_status=$redis->get("event_{$eventId}_status");
        //优先处理预购房间
        if($evrnt_status==0)
        {
            ...
            $redis->set("event_{$eventId}_status",1);
        }
        //获取房间信息
        ...
        //用户是否已购买   
        $isMemberOrdered = $redis->sIsMember("event_order_house_{$eventId}_room_order_phone",session('phone'));
        if ($isMemberOrdered )
        {  
            $this->error('每人最多购买一套!');
        }
        ...
        if(count($ygrooms)>0)
        {
            foreach ($ygrooms as $ygroom) {
                $oldroom=$redis->hGetAll("event_order_house_{$eventId}_room_{$ygroom}");
                if($oldroom['schedule_phone']==session('phone'))
                {
        ...
        */
        ...
        else
        {
            if(!empty($room['schedule_phone']) && $room['schedule_phone']==session('phone'))
                $getLock = 1;//$redis->setnx("event_order_house_{$eventId}_room_{$id}_locked",1);
        }
        ...
        */
        ...
        $this->success(['预购成功',$obj['code'],$orderedRooms]);
    }
    ...
    public function send_sms_code(){
        ...
        $where=['project_id'=>$event['project_id'],'batch_id'=>$event['batch_id'],'customer_phone'=>$phone];
        ...
        if($redisDriver->get("event_order_house_{$eventId}_{$phone}_code") && cookie('order_house_code')){
        ...
        */
        ...
        if ($status1)
        {
            ...
            $redisDriver->set("event_order_house_{$eventId}_{$phone}_code",1,300*1);
            $redisDriver->set("dlsx_order_house_{$eventId}_{$phone}",session_id(),60*60*6);
    ...
    public function check(){
        ...
        if(cookie('phone') != $phone || empty(cookie('order_house_code')))
        {
            $this->error('请先获取验证码');
        }
        if(empty($code))
        ...
        if($loginerror>=5){
            $event = D('Common/EventOrderHouse')->getOneById($eventId);
            $data1["status"]=0;
            D("Choose")->where("customer_phone=".$phone ." and project_id=".$event['project_id']." and batch_id=" .$event['project_id'])->save($data1);
            $this->error('账号锁定，6小时内不能登录');
        }
        if ($code != cookie('order_house_code') && cookie('phone') == $phone){    
        ...
        $this->success("验证成功",session('order_house_jump'));
    }
