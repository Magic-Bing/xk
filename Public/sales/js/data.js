 ...
 * @author zlw
 */
function get_project_room_list($url) {	
	var $project_id = $(".saler-project-id").attr("data-project-id");
	if ($project_id == undefined) {
		layer_alert('请求错误，项目不存在！');
		return false;
	}
	
	var $build_id = $(".saler-project-id").attr("data-build-id");
	var $unit_id = $(".saler-project-id").attr("data-unit-id");
	
	$(".js-saler-project-view-content-selected-wrapper").hide();

	$.ajax({
		url: $url,
		data: {
			info: 'p' + $project_id + 'b' + $build_id + 'u' + $unit_id,
		},
		type: 'POST',
		dataType: 'JSON',
		success: function (data, status) {
			...
			$(".saler-project-view-content-rooms-table").html($rooms.trim());
		},
		error: function (data, status, e) {
			layer_alert('提交连接失败！');
		}
