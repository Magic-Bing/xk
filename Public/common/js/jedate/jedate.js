...
;(function(win) {
	...
	*/
    ...
    //显示隐藏层
    jeDt.shdeCell = function(type, elem) {
        type ? elem.style.display = "none" : elem.style.display = "block";
    };
    //删除当前
    jeDt.remove = function(elem) {
        if (elem == undefined) {
			return true;
		}
    ...
    var jeDate = function(options) {
		...
		var options = jeDt.extend(newConf, options || {});
		
		var dateCell = QD(options.dateCell);
		jeDt.each(dateCell, function(i, cls) {
			new InitDate(options || {}, cls);
		});
		
		return this;
    };
	
	var InitDate = function(options, cls) {
		this.config = options;		
		this.cls = cls;
		this.init();
	};
    ...
    InitDate.prototype = {
        init:function() {
            ...
            elem = opts.dateCell ? self :target;
		...
		setSkin: function(skin) {
            ...
            if (!QD(".jedate-skin-" + skin)[0]) {
                dateDiv.className = Cell.replace("#", "") + ' jedate-skin-' + skin; 
				dateDiv.id = Cell.replace("#", "");
